...
# Encapsulation class for the log dataset, determining feature vectors through options seq/qua/sem
    ...
    def __getitem__(self, idx):
        ...
        if self.quan:
            log['Quantitatives'] = torch.tensor(self.Quantitatives[idx],
                                                dtype=torch.float)
        if self.sem:
            log['Semantics'] = torch.tensor(np.array(self.Semantics[idx]), dtype=torch.float)
